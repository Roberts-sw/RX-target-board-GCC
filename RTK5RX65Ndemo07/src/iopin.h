...
CCALL void iopin_dir(eIOPIN iopin, uint8_t asoutput);
	//HW 22 + #define IO_??
CCALL void iopin_init(eIOPIN iopin, uint8_t iomode);
	//HW 23, p.830-892
CCALL void iopin_mpcfunc(eIOPIN iopin, uint8_t mpcfunc);
...
public:
	...
	~IoPin ()	{}
	void dir (uint8_t asoutput )	{ iopin_dir(this->nr,asoutput);	}
	void init (uint8_t mode ) 		{ iopin_init(this->nr,mode);	}
	void mpcfunc (uint8_t func)		{ iopin_mpcfunc(this->nr,func);	}
