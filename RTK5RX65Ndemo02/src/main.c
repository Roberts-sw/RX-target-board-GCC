...
#define PLLMUL2	(32)	//choose from 20-60, mul = choice/2
...
{	//clock_source_select();
	...
	SYSTEM.PLLCR.WORD=0x1f11;//PLLMUL2-1<<8 | 1<<4 | PLIDIV_-1;	//(source HOCO)
...
# elif(ICK_HZ >  50*1000*1000)
	...
	//bus clock division: necessary: f_PLL=128M, ICLK=32M
	SYSTEM.SCKCR.LONG= 1<<23 | 1<<22			//PSTOP1|PSTOP0
	| CK_4<<28 | CK_4<<24 | CK_4<<16			//FCK ICK BCK
	| CK_4<<12 | CK_4<< 8 | CK_4<< 4 | CK_4<<0;//PCKA -B -C -D

	//USB clock: not used
//	SYSTEM.SCKCR2.WORD=0x0011;	//0008 0024h:0011

	//select PLL as clock source, then stop LOCO
//	IOREG_WRITE(SYSTEM.SCKCR3.WORD,0x400);
...
#	undef _
	...
	TMR0.TCORA=128-1;			//match-frequency = f_clock / 128
	...
	//start timers
	TMR1.TCCR.BYTE=0x18;		//clock source = CMIA0
	TMR0.TCCR.BYTE=0x08;		//clock source = PCLKB
}
