...
uint32_t volatile millis( void );	//Arduino-style timing
	...
	--------------------------------------------------------- */
	//single object:
void led_blink (Sled *self, uint8_t nr)
{	self->next=nr;
}
void led_drive (Sled *self, uint8_t on)
{	iopin_write(self->pin,self->off^on);
}
void led_flash (Sled *self, uint8_t nr)
{	if(!self->patt&&nr<8)
...
void led_run (Sled *self, uint8_t period, uint8_t cnt)
{	if(!cnt)
		self->now=self->next;
	if(self->patt)//if(0)//
